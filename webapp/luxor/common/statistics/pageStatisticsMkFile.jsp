...
<%
	...
	List<PageStatisticsVO> list = (List<PageStatisticsVO>) request.getAttribute("list");
	...
	try {
	//System.out.println(":::::getPageStatisticsMkFile:::list.size()::" + list.size());
	if (list.size() > 30000) {
%><script>
	alert("Please reduce under 30000 rows");
</script>
<%
	}
	/*================== Start DownLoad ==========================================*/
	String sFileName = "PAGE_STATISTICS_" + todayDate;
	...
	for (int i = 0; i < list.size(); i++) {
		idx = 0;
		label = new Label(idx++, i + 1, list.get(i).getPageId(), rowStyleRight);
		sheet.addCell(label);
		label = new Label(idx++, i + 1, list.get(i).getMessageName(), rowStyleCenter);
		sheet.addCell(label);
		label = new Label(idx++, i + 1, list.get(i).getAccessUserId(), rowStyleRight);
		sheet.addCell(label);
		label = new Label(idx++, i + 1, list.get(i).getAccessUserName(), rowStyleRight);
		sheet.addCell(label);
		label = new Label(idx++, i + 1, list.get(i).getLoginIp(), rowStyleRight);
		sheet.addCell(label);
		label = new Label(idx++, i + 1, list.get(i).getAccessTime() + "", rowStyleRight);
		sheet.addCell(label);

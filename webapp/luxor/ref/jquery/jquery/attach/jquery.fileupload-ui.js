...
(function ($) {
    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
        ...
        options: {
    	    unacceptUse: false, //@add 2013.11.21
            ...
            add: function (e, data) {
    			...
    			if(bal_overLimitSize) { //acube customizing
					var _attachmax = 10*1024*1024; //default 10MB
					if(upload_attachmax) {
					  _attachmax = upload_attachmax;
					}
					
					var totalsize = 0;
					  for(var i=0;i<data.files.length;i++) {
					    totalsize += parseInt(data.files[i].size);
					    data.files[i].thumbnail_url = context_root + '/collaboration/ref/images/attachment' + AttachmentUtil.getIcon(data.files[i].name);
					    data.files[i].sizef = AttachmentUtil.getKB(data.files[i].size); // @add 파일 사이즈 체크 추가 2013.10.17
						//data.files[i].sizef = that._formatFileSize(data.files[i]);
					  }
					 if( totalsize > _attachmax) {
					  alert(bal_overLimitSize);
					  try { 
					    failSubmit();
					  } catch(e){
					  }
        ...
        _renderDownload: function (files) {
            ...
            tmpl.find('.download button').button({
                text: false,
